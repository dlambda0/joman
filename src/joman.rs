...
pub fn read_file(file_path: &str, key_path: &str) -> Result<String, Box<dyn Error>> {
    ...
    let plaintext_str = encryption::hyb_decrypt(encrypted_str, &priv_key)
        .map_err(|e| format!("Failed to decrypt entry: {}", e))?;

    Ok(plaintext_str)
}

pub fn zip_file() -> Result<(), Box<dyn Error>> {
    if !Path::new("Journal").exists() {
        return Err("Journal not initialized. Please run 'joman init' first.".into());
    }

    let zip_path = "Journal.zip";
    ...
    if status.success() {
        println!("Journal directory zipped successfully to {}", zip_path);
    } else {
        return Err("Failed to zip the Journal directory.".into());
    }
